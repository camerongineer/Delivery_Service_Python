...
class TestTimeConversion(TestCase):
    ...
    def test_convert_miles_to_time(self):
        miles = 18.0
        start_time = time(hour=8)
        assert TimeConversion.convert_miles_to_time(miles, start_time) == time(hour=9)
        assert TimeConversion.convert_miles_to_time(miles, start_time) != time(hour=9, minute=1)
        miles = 36.0
        assert TimeConversion.convert_miles_to_time(miles, start_time) != time(hour=9)
        assert TimeConversion.convert_miles_to_time(miles, start_time) == time(hour=10)
