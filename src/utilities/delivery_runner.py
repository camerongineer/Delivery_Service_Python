...
def _display_awaiting_package_message(truck, package):
...
def _reload_for_next_run(truck, run, fast_reload=False):
...
def _display_address_update_message(address_update_message):
...
def _display_returned_to_visited_location_message(truck):
...
def _display_starting_route_message(truck):
